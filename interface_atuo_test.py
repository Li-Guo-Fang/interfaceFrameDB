    ...
    for id, api in enumerate(api_list, 1):
        ...
        for idx, case in enumerate(api_case_list, 1):
            ...
            if  code == int(protocol_code):
                ...
                info("接口[%s]第%s条用例的依赖数据存储成功！"%(api_name,idx))
                # 接下来进行结果校验
                error_info = CheckResult.check(responseObj.json(),check_point)
                if error_info:
                    debug("接口[%s]第%s条用例校验结果失败，失败信息：%s"%(api_name, idx,str(error_info)))
                else:
                    info("接口[%s]第%s条用例运行成功！"%(api_name, idx))
            else:
                debug("接口[%s]的第%s用例响应协议code=%s，不是期望值code=%s"%(api_name,idx,code,protocol_code))
